...
| FR-09 | 行政還元ダッシュボード | **実装済み**（概況・町丁目・CSV 出力は実測。地図とスフィア基準の判定は FR-05／FR-06 待ちで想定値） |
| FR-09-4 | 自治体標準オープンデータセット準拠の CSV 出力 | **実装済み**（検証済み／未取得項目／町丁目の3種） |
| FR-12-5 | エリア×項目の充填率の可視化 | **実装済み**（カテゴリごとの空白） |
...
LINE ログインが完走しないため、**ログインが要る機能（チェックインの保存・カード）は
そのままでは確かめられません**。ローカル起動のときだけ使える入口を用意しています。

| 入口 | 内容 |
| :--- | :--- |
| 起動画面の**「開発用ログイン」** | LINE ログインと同じ形のセッションを発行します。以降の経路は本番と同じです |
| **http://localhost:8787/card-catalog.html** | カードの種類と中身の一覧（全395枚）。出題やアイテムを増やすと自動で追随します |
| **http://localhost:8787/caps.html** | 端末機能（音・振動・画面点灯維持）の実測ページ |

**どちらも本番には出ません。** 経路が生えるのは `USE_FAKE_DATASTORE` が真のとき
（＝データストアへ接続していないとき）だけで、本番はこれを設定しない運用です。
`card-catalog.html` はデプロイ用の ZIP にも含めていません。

チェックインを繰り返して確かめたいときは、`.env` の `CHECKIN_COOLDOWN_HOURS` を
`0` にすると再チェックインの制限が外れます（**デモ前に戻すこと**）。
