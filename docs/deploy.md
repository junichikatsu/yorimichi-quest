...
| 2 | データストアのテーブルを5つ作成（キーは下記） | テーブル ID × 5 |
...
| **チェックインとクイズだけ 500、他は正常** | `checkins` / `user_spot_state` テーブルまたは UUID の作成漏れ（FR-03・FR-04 で追加） | テーブルを作り、UUID を envVars へ |
