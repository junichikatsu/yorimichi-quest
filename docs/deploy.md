...
**既定では 50 件ずつ入れる。** 一息に全件入れない理由が2つある。

1. 実行環境のタイムアウトに当たる（370件で7〜18秒）
2. **データストアのアクセス数に月次上限がある**（プラットフォーム制約 E4。フリー枠は
...
| `delayMs` | 0 | 1件ごとの間隔。書き込みが速すぎて弾かれる場合に効かせる |
...
| 投入が途中で止まる | 速度または上限 | `delayMs=100` を付け、`count` を小さくして `nextOffset` から再開 |
