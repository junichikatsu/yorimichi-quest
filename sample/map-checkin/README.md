...
| `pnpm ingest:boundaries` | 町丁目境界256区画を取り込み（e-Stat 国勢調査 小地域境界） |
...
キャラクター（髪型10種・服10種・装備10種）と**カードの全パターン**を並べた確認ページがあります。
**サーバー不要**で、ビルド後に `apps/web/public/avatar-preview.html` をブラウザで直接開くだけです。

カードは本体と同じ CSS を読み込んでいるので、ここで見たとおりの見た目が画面に出ます。
達成・未達成の両方を並べてあります。
