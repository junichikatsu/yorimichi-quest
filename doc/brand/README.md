...
- 地：`#6b4e8f`（`styles.css` の `--accent`）
- 図案：白

**ファビコンと同じ図案である**（`apps/web/public/index.html` / `dashboard.html`）。**片方だけ直さないこと。** 図案が分かれた時点で、タブとLINEで別のサービスに見える。
...
- **ダッシュボードの色（`#2d6ca2`）版は作っていない。** LINE に上げるのはミニアプリのアイコンだけである
